// validators.js — validación clínica con UI + alert()
...
export function calcularDV(num){let s=0,m=2; for(let i=num.length-1;i>=0;i--){s+=Number(num[i])*m; m=(m===7)?2:m+1;} const r=11-(s%11); return r===11?'0':r===10?'K':String(r)}
...
export const roundQ = (n)=> Math.round(Number(n)*4)/4;
...
export function clampIntInRange(n,min,max){ if(n==null||n==='') return null; const v=parseInt(n,10); if(!Number.isFinite(v)) return null; return Math.max(min, Math.min(max, v)); }
export function formatCylRaw(n){ if(n==null||n==='') return null; const v=-Math.abs(Number(n)); return Number.isFinite(v)? roundQ(v) : null; }
export function formatAddRaw(n){ if(n==null||n==='') return null; const v=Math.abs(Number(n)); return Number.isFinite(v)? roundQ(v) : null; }

function msgBox(prefix){ return document.getElementById((prefix||'') + 'rxMsg'); }
function showMsgs(prefix, hard, soft){
  const box = msgBox(prefix);
  if (!box) return;
...
export function validateRx(prefix='', form){
  ...
  // Alertas: errores duros bloquean; warnings informan sin bloquear
  if (hard.length){
